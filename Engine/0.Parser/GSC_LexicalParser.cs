...
namespace GSC_Engine
{
    ...
    public static class GSC_LexicalParser
    {
        ...
        private static GSC_ScriptToken Handler()
         => new GSC_ScriptToken(GSC_ScriptTokenType.HANDLER, "$");
        ...
        public static List<GSC_ScriptToken> Parse(string script)
        {
            ...
            string[] splitted = script.Split(' ', StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (string parts in splitted)
            {
                ...
                else
                {
                    if (OpenBracket == false)
                    {
                        if (part.StartsWith("@"))
                        {
                            tokens.Add(IsValidString(part[1..]) ? Keyword(part) : Undefined(part));
                        }
                        else if (part.StartsWith("["))
                        {
                            ...
                            {
                                if (!IsValidString(part[1..])) UndefinedBracketed = true;
                        ...
                        else if (part.EndsWith(";"))
                        {
                            ...
                            if (IsValidString(endPart))
                            {
                                tokens.Add(String(endPart));
                                tokens.Add(End());
                                return tokens;
                            }
                        }
                    ...
                    else //Open bracket is true
                    {
                        ...
                        else if (part.StartsWith("@") || part.StartsWith("[") || int.TryParse(part, out _) || part.EndsWith(";"))
                        {
